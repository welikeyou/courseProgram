...
public class GoodsDaoImpl implements GoodsDao {
	...
	public boolean addGoods(Goods goods) throws Exception {
		pstmt = null;
		String sql = "insert into goods(gname,gphoto,types,producer,price,carriage,pdate,paddress,described,gnumber)values(?,?,?,?,?,?,?,?,?,?)";
		int result = 0;
	...
	public boolean editInfo(Goods goods) throws Exception {
		pstmt = null;
		String sql = "update goods set gname=?,gphoto=?,types=?,producer=?,price=?,carriage=?,paddress=?,described=?,gnumber=? where gid="
				+ goods.getGid();
		int result = 0;
		pstmt = this.conn.prepareStatement(sql);
		pstmt.setString(1, goods.getGname());
		...
		pstmt.setString(3, goods.getType());
		pstmt.setString(4, goods.getProducer());
		pstmt.setInt(5, goods.getPrice());
		pstmt.setInt(6, goods.getCarriage());
		pstmt.setString(7, goods.getPaddress());
		pstmt.setString(8, goods.getDescribed());
		pstmt.setInt(9, goods.getNumber());
		result = pstmt.executeUpdate();
		pstmt.close();
		if (result == 1) {
			return true;
	...
	public int queryNumberById(int gid) throws Exception {
		ResultSet rs = null;
		String sql = "select gnumber from goods where gid =?";
		int number = 0;
		pstmt = this.conn.prepareStatement(sql);
		pstmt.setInt(1, gid);
		rs = pstmt.executeQuery();
		if (rs.next()) {
			number = rs.getInt("gnumber");
		}
		pstmt.close();
		rs.close();
		return number;
	...
	public List<Goods> getLatestGoods(int gid, String type) throws Exception {
		...
		Goods goods;
		ResultSet rs = null;
		String sql = "select gid,gname,price from goods where gid<>? and types=?";
		pstmt = this.conn.prepareStatement(sql);
		pstmt.setInt(1, gid);
